...
def renderHyperboloid(cx, cy, cz, a, b, p):
    glColor4f(1.0, 1.0, 1.0, transparency)  # Set transparency here
    PI = math.pi
    TWOPI = 2 * PI
...
def display(window):
    ...
    renderHyperboloid(0, 0, dist, 0.8 * dist * 0.7, 0.6 * dist * 0.7, 40)  # Adjust parameters for the hyperboloid
...
def key_callback(window, key, scancode, action, mods):
    global x_angle, y_angle, scale, animation_mode, fi, tetha, dist, transparency
    ...
    if action == glfw.PRESS or action == glfw.REPEAT:
        ...
        if key == glfw.KEY_LEFT:
            glRotatef(2, 0, 1, 0)  # Rotate left around y-axis
        if key == glfw.KEY_RIGHT:
            glRotatef(-2, 0, 1, 0)  # Rotate right around y-axis
        if key == glfw.KEY_UP:
            dist += 0.1
            scale += 0.05
