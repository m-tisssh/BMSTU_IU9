...
-- Представления
DROP VIEW IF EXISTS CourseLessons;
GO
...
AS
BEGIN
    IF UPDATE(userID) OR UPDATE(transactionNumber) 
    BEGIN
...
-- Добавление таблицы OrderCourse
CREATE TABLE OrderCourse (
    userID INT NOT NULL,
    courseID INT NOT NULL,
    transactionNumber INT NOT NULL,
    PRIMARY KEY (userID, courseID, transactionNumber),
    CONSTRAINT FK_OrderCourse_User FOREIGN KEY (userID) REFERENCES Users(userID),
    CONSTRAINT FK_OrderCourse_Course FOREIGN KEY (courseID) REFERENCES Courses(courseID),
    CONSTRAINT FK_OrderCourse_Transaction FOREIGN KEY (transactionNumber) REFERENCES Orders(transactionNumber)
);
GO

-- Триггер для пересчёта totalPrice 
CREATE TRIGGER trg_RecalculateTotalPrice
ON OrderCourse
AFTER INSERT, DELETE
AS
BEGIN
    ...
    SET totalPrice = (
        SELECT SUM(c.price)
        FROM Courses c
        INNER JOIN OrderCourse oc ON c.courseID = oc.courseID
        WHERE oc.transactionNumber = Orders.transactionNumber
    )
    FROM Orders
    WHERE Orders.transactionNumber IN (
        SELECT DISTINCT transactionNumber FROM inserted
        UNION
        SELECT DISTINCT transactionNumber FROM deleted
    );
END;
GO
...
HAVING AVG(totalPrice) > 100; -- Условие: только методы оплаты со средней стоимостью заказов > 100
...
HAVING SUM(totalPrice) > 100; -- Условие: только статусы с суммарным доходом > 100
